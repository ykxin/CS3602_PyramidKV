...
from pyramidkv.gptneox_model import gptneox_attn_forward_PyramidKV
...
def replace_gptneox(method):
    if method == "pyramidkv":
        print("Using PyramidKV for GPT-NeoX!")
        try:
            transformers.models.gpt_neox.modeling_gpt_neox.GPTNeoXAttention.forward = gptneox_attn_forward_PyramidKV
        except AttributeError:
            pass
            
        try:
            transformers.models.gpt_neox.modeling_gpt_neox.GPTNeoXSdpaAttention.forward = gptneox_attn_forward_PyramidKV
            print("Patched GPTNeoXSdpaAttention")
        except AttributeError:
            pass

        try:
            transformers.models.gpt_neox.modeling_gpt_neox.GPTNeoXFlashAttention2.forward = gptneox_attn_forward_PyramidKV
